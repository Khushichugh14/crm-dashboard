...
export default function EmployeeTable({ employees, onEdit, onDelete }:
  { employees: Employee[]; onEdit: (e: Employee) => void; onDelete: (id: string) => void; }) {
        ...
        <tbody>
          {employees.map((e, i) => (
            <tr key={e.id} className="odd:bg-white/10 even:bg-transparent">
              ...
              <td className="py-3 text-sm text-slate-700 flex items-center gap-2">
                {e.teamLogo && <img src={e.teamLogo} alt={e.team} className="h-7 w-7 rounded-md object-contain" />}
                <span>{e.team}</span>
              </td>
