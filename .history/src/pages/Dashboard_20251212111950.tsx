...
/**
 * NOTE:
 * - THEME defined locally (not exported) so the file still exports a React component and Fast Refresh works.
 * - Adjust asset import paths if your structure differs.
 */
const THEME = {
...
export default function Dashboard(): JSX.Element {
  ...
  // placeholder handlers — replace with real ones if needed
  function onEdit(e: any) {
    // open modal or navigate to edit form
    // console.log("edit", e);
  }
  function onDelete(id: string) {
    // remove or send delete request
        ...
        <main className="col-span-9">
          ...
          <section className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              {/* IMPORTANT: set a maxHeight so the table scrolls internally and does not shrink */}
              <div className="overflow-auto" style={{ maxHeight: "calc(100vh - 220px)" }}>
                <EmployeeTable employees={employees} onEdit={onEdit} onDelete={onDelete} />
              </div>
            </div>

            {/* RIGHT: smaller stats + smaller banner + sticky behavior */}
            <div style={{ position: "relative" }}>
              <div style={{ position: "sticky", top: 24 }}>
                {/* Team statistics — reduced padding / smaller cards */}
                <div
                  style={{
                    background: THEME.cardBg,
                    border: `1px solid ${THEME.cardBorder}`,
                ...
                {/* Constrain entire right column's content to a reasonable height so it doesn't push the left */}
                <div style={{ maxHeight: 410, overflow: "hidden" }}>
                  {/* Only ONE training banner here (smaller variant) */}
                  <div
                    style={{
                  ...
                  >
                    ...
                    <div style={{ width: 92, flexShrink: 0 }}>
                      {/* If your TrainingBanner component already provides an image/illustration, you can render it here.
                          For now we show a placeholder circle avatar to match the smaller style. */}
                      <div
                        style={{
                          width: 92,
                          height: 92,
                  ...
                  {/* OPTIONAL: additional small widget or filler (keeps right column compact) */}
                  <div style={{ height: 12 }} />

                  {/* (Don't render the banner twice) */}
                </div>
              </div>
            </div>
          </section>
